...
for (const width of [320, 390, 768, 1440]) {
  test(`Week 2 shows only previewable media and fits a ${width}px screen`, async ({ page, request }) => {
    ...
    await expect(page.locator('.project-introduction p')).toHaveText("While this is a very simple design, it was hard to get the form factor I liked the most. The idea is a little CTA tracker powered by an AMOLED display + ESP32 module I found online. I've also been wanting to learn how to use BLE for communication instead of Wi-Fi, so I am hoping this project could make great use of it.");
    await expect(page.locator('.project-image-block').first().locator('.project-reflection')).toHaveText("I mocked up and drew this little CTA UI in pixelart.com, and then did some \"poor-mans physics\" to get the frosted case look that I wanted, basing the frost on what a resin printer can realistically do. So while it was super easy to design, I had to go through like 5 form factors and frosted appearances before I got the case and UI to look how I wanted it to. And like I said, I love simplicity.");
